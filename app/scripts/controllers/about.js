...
angular.module('appGeoApp')
  .controller('AboutCtrl', ["$scope", "$rootScope", "$location", "$timeout", "MarkersService", "$interval",
      ...
      var routeByVendor = function (vendor, day) {
        ...
        if (ddd == "LUN") {
          ...
          for (var i = 0; i < $rootScope.markers.length; i++) {
              ...
              if (marker.vendor == vendor && marker.day == day && marker.map != null) {
              ...
              chart = new Chart(document.getElementById("pieChart").getContext("2d"), {
                      ...
                      data: {
                          datasets: [{
                              backgroundColor: ["#0B9B00","#FF1E00","#B0B0B0", "#000000"],
                              labels: ["Ventas", "No Ventas", "Visitas Pendientes", "Fuera de Ruta"],
                              data: [$rootScope.ventas, $rootScope.noVentas, $rootScope.planificacion, $rootScope.ventasNo]
                        }]
                      },
                      options: chartOptions
              });
